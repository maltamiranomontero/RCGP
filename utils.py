...
def add_noise_cov(K: tf.Tensor, M: tf.Tensor, likelihood_variance: TensorType) -> tf.Tensor:
    """
...
def add_likelihood_noise_cov(K: tf.Tensor, M: tf.Tensor, likelihood: Gaussian, X: TensorType) -> tf.Tensor:
    """
    Returns K + σ², where σ² is the likelihood noise variance.
    """
    return add_noise_cov(K, tf.squeeze(M, axis=-1), tf.squeeze(likelihood.variance_at(X), axis=-1))