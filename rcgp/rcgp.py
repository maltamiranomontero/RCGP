...
class RCGPR_no_bias(RCGPR_deprecated):
    @inherit_check_shapes
    def predict_f(
        self, Xnew: InputData, full_cov: bool = False, full_output_cov: bool = False
    ) -> MeanAndVariance:
...
class RCGPR_no_noise(RCGPR_deprecated):
    @inherit_check_shapes
    def predict_f(
        self, Xnew: InputData, full_cov: bool = False, full_output_cov: bool = False
    ) -> MeanAndVariance:
        ...
        knn = self.kernel(Xnew, full_cov=full_cov)
        kmn = self.kernel(X, Xnew)
        kmm_plus_s = gpflow.utilities.add_likelihood_noise_cov(kmm, self.likelihood, X)
        ...
        # Compute the projection matrix A
        Lm_plus_s = tf.broadcast_to(Lm_plus_s, tf.concat([leading_dims, tf.shape(Lm_plus_s)], 0))  # [..., W, W]
        A = tf.linalg.triangular_solve(Lm_plus_s, kmn, lower=True)  # [..., W, N]
        ...
        f = err - 2*likelihood_variance*W_dy/W
        B = tf.linalg.triangular_solve(Lm_plus_s, f, lower=True)  # [..., W, N]
        f_mean_zero = tf.linalg.matmul(A, B, transpose_a=True)
